...
/**
 * docs-check.js — Root repository documentation validator
 *
 * DIVERGENCE NOTE: This is an independent adaptation for the root repo's
 * directory structure. It is NOT kept in sync with src/.opencode/docs-check.js.
 * The root repo has a simpler structure (only docs/decisions/ and ktlo/ exist
 * at root); the src/ copy validates a richer tree.
 *
 * Validates the root-level documentation structure:
 * - docs/decisions/
 * - ktlo/
 * - references/
 *
 ...
 * - Filenames match conventions (docs/ and ktlo/ only)
 * - Required Last modified metadata exists and is ISO format (docs/ and ktlo/ only)
 * - Decision record and ktlo statuses are valid (docs/ and ktlo/ only)
...
// Directories that follow the full document lifecycle (filename conventions, status, etc.)
// Root only has decisions/ and ktlo/; other subdirs (specs, plans, architecture,
// implementation-maps, contracts) exist only in src/.
const LIFECYCLE_DIRS = [
  path.join(DOCS_DIR, "decisions"),
  KTLO_DIR,
];

// All directories to scan for index/link checks
const ALL_DOC_BASE_DIRS = [DOCS_DIR, KTLO_DIR, REFERENCES_DIR];
...
// Filename conventions are defined above (DOCS_FILENAME_RE, KTLO_FILENAME_RE).
...
function checkFilenameConventions(dir, type) {
  ...
  for (const file of mdFiles) {
    ...
    if (type === "decision") {
      // Docs files: YYYY-MM-DD-description.md
      if (!DOCS_FILENAME_RE.test(file)) {
        error(
          filePath,
          "filename-convention",
          `Filename "${file}" does not match docs pattern YYYY-MM-DD-description.md`
        );
      }
    } else if (type === "ktlo") {
...
function main() {
  // Collect all markdown directories across all doc bases
  const allDirs = [];
  for (const baseDir of ALL_DOC_BASE_DIRS) {
    if (fs.existsSync(baseDir)) {
      allDirs.push(...getAllMarkdownDirs(baseDir));
    }
  }

  // Determine which directories are lifecycle-managed
  const lifecycleDirSet = new Set(LIFECYCLE_DIRS.map((d) => path.resolve(d)));

  // Run per-directory checks
  for (const dir of allDirs) {
    // Index and link checks apply everywhere
    ...
    // Lifecycle checks (filename conventions, metadata, status) only for managed directories
    if (lifecycleDirSet.has(path.resolve(dir)) && type) {
      checkFilenameConventions(dir, type);
      checkLastModified(dir);
      checkStatus(dir, type);
      checkGherkinScenarios(dir);
    }

    // Mermaid and JSON checks apply everywhere
    checkMermaid(dir);
    checkJsonArtifacts(dir);

    // Check document links (all files including index)
    const mdFiles = getMarkdownFiles(dir);
    for (const file of mdFiles) {
