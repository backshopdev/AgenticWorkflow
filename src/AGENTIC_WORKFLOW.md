...
Workflow reference template. For a concise introduction and quick start, see
[AGENTS.md](AGENTS.md).
...
- `@orchestrator` is the human-facing planner/router and never edits files.
...
- `@review-agent` is a reusable read-only reviewer. Launch one independent
  session per meaningful perspective, in parallel.
